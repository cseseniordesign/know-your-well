...
export default function ViewWell() {
    ...
    if (formElements) {
        formElements['county_id'] = countyOptions.find(option => option.key === (formElements['county_id']-1).toString()).value; // -1 to properly index
        formElements['nrd_id'] = nrdOptions.find(option => option.key === (formElements['nrd_id']-1).toString()).value;
